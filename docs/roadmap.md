...
### Step B2: supply answers the price signal
...
- **Done: firms now buy capacity on their own.** A new monthly stage advances open construction and then reviews the same production evidence the management stage uses. A solvent firm that ran out of stock more often than it held unsold goods, expects a positive operating cash margin, has demand absorbing everything its current capacity can make, and has no construction under way, commits its own cash and orders a works. The size of the expansion is not a tuned constant: it is the smaller of the demand the firm watched walk away and the capacity it can pay builders for out of cash, so a poor firm in a hungry market honestly stays small. Committing the money is a scoped `ProposeMajorInvestment` power crossing `WorldCommand::CommitFirmInvestment`, and the budget is paid to local households as building income on the closed-circuit rule. Gated in both directions: a profitable sold-out bakery must expand and pay its builders, and the loss-making production chain must refuse to expand however hungry its market is. ADR 0172; `SIMULATION_VERSION` is now 99.
- **Measured after this step.** Seed 1, 50 years: fingerprint `16267805333852176209`, 6.04 s total, 120.7 ms per simulated year.
- **What the long run still shows, and the next diagnosis.** The demo economy remains fatal: survival consumption reads 0.00% from the very first years, output decays through the 2040s and reaches zero by 2047, after which the world is an empty shell of politics with no production at all. The new capacity stage never fires in that run, and the reason is visible in the chronicle: the bakeries cannot procure grain, and firms operate at a cash loss, so no firm ever meets the profitability test for expansion. Cheap capacity was never the binding constraint. The next loop to close is why a chain that households are hungry for cannot cover its own wage bill — whether households simply have no money to bid with, or the wage is set without reference to what the output can fetch.
- **Known gaps carried forward.** Construction takes a fixed three months because builders are not yet an industry with their own order book. Expansion is cash-financed only, so the credit market plays no part in the investment cycle. Firm entry still triggers only on survival-good shortages, so a durable high price in any other good attracts no new entrant.
- **Next in this step:** widen firm entry beyond survival goods to any persistently dear good with a solvent founder, and extend the `fiscal_circuit` invariant to account for `actor_cash` and `committed_investments` so that investment leaks cannot reopen unnoticed.

### Step B3: the employer gets a side of the wage bargain

- **Diagnosis.** Wages were priced entirely from the worker's side. `competitive_labor_bid` takes the cohort's past income per person per month, marks it up by the firm's staffing urgency, and adjusts it by regional vacancy or unemployment pressure. Nothing in that expression asks what the work is worth. The only thing standing between a firm and a ruinous hire was whether it had cash for this month's wage, which every firm does right up until it does not. That is a market with one side, and it is the reason the demo economy has no profitable firm and therefore no expansion and no entry.
- **Done: a firm refuses a hire it cannot earn back.** A hire is now judged against the value of the work bought: the local observed price of a batch of output, less the local observed price of every input the recipe consumes, divided by the labor the recipe says a batch takes. Above that ceiling the vacancy stays open rather than being filled at a loss. The bound is owned on both sides: the firm would pay for a bad hire in cash every month, and the worker pays for it by staying unemployed because the available work is not worth the asking wage. Where the output or an input has no observed local price there is no evidence and no ceiling. Gated in both directions: a bid that outruns the work leaves the vacancy open, and the very same worker is hired once the output price covers the wage. ADR 0173; `SIMULATION_VERSION` is now 100.
- **Honest result.** The demo fingerprint is unchanged, because the demo scenario never enables the labor market for its recipes and its firms carry static rosters written into the content. This slice closes the loop for hires made from now on; it does not touch a wage already agreed.
- **Next in this step:** the same arithmetic must reach existing agreements. A firm whose payroll outruns its output has no way to lay off, renegotiate, or wind down, so it simply bleeds until production stops - which is precisely what the fifty-year chronicle shows. After that, apply the ceiling to poaching, retention, and switching, and enable the labor market in the demo content so its staffing is a market outcome rather than a fixture.

### Step B4: a standing wage is judged by the same arithmetic as a new one

- **Diagnosis.** ADR 0173 closed the loop only for hires not yet made. Every agreement already in force was untouched, and the world had no corrective move between "perfectly fine" and "ruined": `plan_cash_constrained_staffing` is advisory and never fires anyone, while distress downsizing in `distress.rs` wakes only after the owner refuses to refinance chronic wage arrears, by which point the business is already dead. So a firm whose payroll outran its output simply bled until production stopped, which is what the fifty-year chronicle records.
- **The first draft was wrong, and that is the interesting part.** The rule as first written ended any agreement whose wage stood above the value ceiling for the current month. It was gated, it passed its own tests, and it broke nearly every other test world in the repository. The instinct was to fix the worlds - raise prices twentyfold here, cut a wage there - and two rounds of that were done before the obvious registered: the worlds were not mistuned, the rule was. A layoff that is instant, total, and free to the employer is a magic correction, not a decision. It is the same failure the *No unowned constraints* rule names, only inverted: not a limit nobody pays for, but an action nobody pays for.
- **Done: the employer waits as long as its own cash lasts.** An agreement is stood down when the wage outruns the value of the work **and** the agreement already carries unpaid arrears. Declining a hire is free; ending a standing agreement is not, because the employer gives up a worker it already found and trained and must court another when the price recovers. So it waits out a bad price on its own money, and the question settles only when the loss stops being financed by the employer and starts being financed by the worker, who never agreed to lend it anything. Accrued arrears survive the release. Reported as a typed `EmploymentEndedAsUnprofitable` event; the stage runs before hiring each month. No "tolerate for N months" constant was needed - the pace is set by the depth of the employer's pocket. Gated in both directions: an unpaid loss-making agreement ends, and a loss-making agreement whose wage is still met survives untouched. ADR 0174; `SIMULATION_VERSION` is now 101.
- **A real leak found on the way.** Extending the `fiscal_circuit` invariant to count every claim on the treasury exposed a twelve-unit discrepancy that is not a measurement artifact: `RegionalHousingConstructionStarted` debits the treasury and credits nobody. It is the same destruction of money that ADR 0170 closed for taxation and ADR 0172 closed for capital works, still open for housing. The invariant now names the channel instead of hiding it.
- **Measurement.** Fifty years, seed 1: fingerprint `2494450776851923311`. The demo trajectory is unchanged in character, because the demo scenario still never enables the labor market for its recipes and pays its wages out of static rosters, so almost nothing there accumulates arrears against a loss-making wage.
- **Next in this step:** pay housing construction to the households that build it, so the treasury stops burning money. Then enable the labor market in the demo content, so both wage rules finally act on the fifty-year run; extend the ceiling to poaching, retention, and switching; summarize releases in the chronicle; and justify or delete `DISTRESS_DOWNSIZING_BPS`, which is still an unowned number.

### Step B5: a dwelling is built by somebody who gets paid

- **Diagnosis, found by an invariant rather than by reading.** Extending the `fiscal_circuit` gate to count every claim on the treasury during a year left a stubborn twelve-unit discrepancy in a world small enough for twelve units to be visible. It was not a measurement artifact: `execute_annual_housing_investment` debited the treasury for a project's committed cost and credited nobody, so the money left the world. That is the same leak ADR 0170 closed for taxation and ADR 0172 closed for founding capital, still open for housing - and it had been sitting there behind an invariant that only checked the annual closure.
- **Done: the building money reaches the builders.** The committed cost is paid to the households of the region that builds, split by population with the same largest-remainder routine used for founding capital - shared rather than copied, because two ways of dividing public money across the same cohorts would eventually disagree and the disagreement would be a rounding leak. Each payment is reported as a typed `HousingOutlayDistributed` event. A region with no households starts no project, since a project with no builders would be the leak under another name. Gated: every unit the treasury spends on building must be earned by a builder, and the payment must be reported rather than silent. ADR 0175; `SIMULATION_VERSION` is now 102.
- **Measurement.** Fifty years, seed 1: fingerprint `18426130385026289368`.
- **Next in this step:** the money still arrives without a job behind it. Nobody is employed to build, the whole sum is paid the moment the project starts rather than as the work proceeds, and ADR 0172 carries the identical gap for capital works. Both point at the same missing thing - construction as a sector with its own firms, recipes, and labor demand - which is also what would let the two wage rules from Steps B3 and B4 bite on a real roster.

### Step B6 (diagnosed, not yet built): scarce supply is handed out by queue order, and the queue starves a region to death

The demo economy collapses to zero output by 2047 with survival consumption pinned at 0.00% from the fourth month onward. This step records what that actually is, because the cause turned out to be three unclosed halves feeding each other rather than one bad number.

- **Who starves, and why.** Only cohort 3 (Eastport) ever goes hungry; Northreach and Southvale sit at 100% fulfilment. Eastport has a bakery but no farm, and lives on grain hauled in over route 1 from Northreach. Every month its bakery reports `FirmProcurementShortfall` for the full order while the road sits nearly empty, because procurement orders are settled in the order `(region, good, buyer)` and local offers are always tried before imports. Northreach and Southvale bakeries empty the grain book before the queue reaches region 3. Eastport bakes through its opening stock, and from month four its people simply have nothing to eat.
- **The queue is a magic constraint.** Registration order decides who eats. Nobody pays for that decision: the Northreach farm sells its grain at the same price either way, so it gives up nothing by serving the cheaper buyer first, and the buyer who could earn more from the same sack has no way to say so. This is exactly the pattern AGENTS.md forbids - a binding limit with no owner - wearing a sort key as a disguise.
- **The starvation then manufactures a war.** `update_bilateral_grievances` accrues 500 bps a month whenever a need goes unmet while a foreign firm was offering that good over a peaceful route. Eastport's monthly shortfall is read as Arcadia withholding grain, so Borealia's grievance climbs from month one, crosses `GRIEVANCE_HOSTILITY_THRESHOLD_BPS` (7_500) after fifteen months, and hostility makes `direct_market_route` return `None`. Import is now not merely last in the queue but forbidden, so the shortage that caused the grievance is guaranteed to continue. The grievance rule is sound in itself; what feeds it is false evidence, because no Arcadian seller ever refused Eastport - the queue did.
- **What was tried and deliberately reverted.** Ranking procurement orders by what a unit of the input is worth to each buyer (revenue of the batch less its other materials, spread over the units consumed - the purchasing twin of the ADR 0173 wage ceiling) was implemented and measured. It correctly reorders the claims (Southvale 24, Eastport 22, Northreach 20 per unit of grain), but the fifty-year run got worse, not better: 73 excess deaths in the very first year and the same collapse, because the hunger simply moved to Northreach. The reason is that ranking alone is only half the circuit. The winner still pays the seller's posted price, so the seller is indifferent, no extra revenue reaches the farms, and nothing pulls new grain into existence. A reshuffled shortage is still a shortage. The change was reverted rather than kept as a partial improvement, and the demo credit-market test that failed under it was left untouched - it was reporting a real behavior change, not being inconvenient.
- **What closing this actually requires, in order.** (1) A buyer facing a shortage bids above the posted price up to its own value ceiling, so scarcity reaches the seller as revenue rather than as a queue position; the loser pays in lost production, and the winner pays in margin. (2) The extra revenue is what makes farm expansion and new farm entry answer the shortage, which is the supply response ADR 0172 already built and this channel never triggers. (3) Grievance evidence must distinguish "the foreign seller had none left" from "the foreign seller would not sell to us"; only the second is a material cause for hostility. Until (1) exists, the demo will keep proving that a region without its own farm cannot survive the sort key.
- **What the displacement-price spike then proved** (branch `spike/contested-input-price`, kept out of `main`). Half (1) was built: a good is marked contested when the month's orders exceed the whole offer book, the contested unit goes to the highest value ceiling, and the winner pays the displaced claimant's bid rather than the posted price, with the difference credited to the seller and published as the observed trade price. It works - the fifty-year fingerprint moves to `18243833979239076188` - and it still does not save anybody. Grain now clears at Southvale 24, Eastport 22, Northreach 20, so Eastport eats and Northreach starves instead, with the same 73 deaths in the first year. The reason is the third missing half, and it is the decisive one: **no new grain can ever appear.** `execute_observed_firm_entry` reads `current_regional_survival_shortages`, which is built from unmet household needs, and the only consumption profile in the demo is bread. Grain is an intermediate good that no household ever eats, so a permanent grain shortage is invisible to firm entry no matter how high the price climbs. The price signal is now correct, loud, and shouting into a channel that has no listener. Raising the price of a fixed quantity only decides who goes without.
- **Therefore the next slice is supply, not price.** Firm entry and capacity expansion must answer a shortage of an *input* the way they answer a shortage of a final good: an unfilled `FirmProcurementShortfall` standing next to a price that exceeds the cost of producing that good is precisely the evidence an entrant needs, and it is already journaled every month. Once a farm can appear in Eastport - or an existing farm can grow because contested grain finally pays for the growth - the displacement price becomes the thing that funds the answer instead of merely ranking the victims. The spike branch should be rebased and merged as one slice with that supply response, not before it.
- **Built (ADR 0176): the shortage is now remembered and entry can see it.** `World` keeps `monthly_firm_input_shortfalls` beside the other monthly firm accounts, procurement accumulates every unfilled order against the buyer's region as it journals the shortfall, and `execute_observed_firm_entry` accrues pressure from that map exactly as it does from an unfed household. Intermediate goods stopped being second-class. First-year excess deaths fall from 73 to 58 and the yearly toll decays faster; fingerprint `8238339567197527992`, simulation version 103.
- **What that exposed, and the next slice.** The demo still starves, and no farm is founded, for one stated reason: `select_entry_worker_cohort` skips every cohort that is not `Unemployed`, and every demo cohort is employed. An entrant here waits for somebody to be idle; in life a new employer takes staff from an existing one by offering more than the work is currently paid. The value ceiling from ADR 0173 already says what a founder could defend bidding, and the labour market already knows how to move a worker between employers. Until entry can outbid an incumbent for staff, the pressure ADR 0176 creates has nowhere to go, and the price signals of the spike branch have nobody to answer them.
